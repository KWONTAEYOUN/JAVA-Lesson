...
public class AnimalCommunity {
	...
	public static void main(String[] args)  {
		System.out.println("~~~~~ 여기는 유기견 동물 분양소 입니다. ~~~~~");
			Animal[] animals = new Animal[10];
			animals[0] = new Puppy("푸들이","브라운");
			animals[1] = new Cat("구름이", "그레이");
	...
	//  동물 보호소 규정(비지니스 로직)
	// 	강아지는 15세이상, 고양이는 18세이상, 토끼는 13세 이상 그 외에는 20세 이상 분양받을 수 있습니다.	
	//  Member 클래스 isAdopt 메소드에서 실행. -> intanceof 연산 사용.
		
		String name="모모";
		int age = 17;
	    Member member = new Member(name, age);		// Member 클래스는 인스턴스 필드는 age, name 2개
		
		StringBuilder sb = new StringBuilder(name);	// String은 문자열 연결결과 다른 문자열이 나오면 객체 새로 만들어 저장
		// String은 불변객체(문자열 변경 못함)
		// StringBuilder는 가변 객체(문자열 변경 가능.문자열 추가 연산 결과를 동일 객체에 저장) => 문자열 연결연상에 사용
		sb.append(" 회원님 \n");				     	// message += " 회원님 \n"
		boolean result;
		for(int i=0;i<animals.length;i++) {
			if(animals[i]==null) 			// 배열요소에 참조값이 더 이상 없으니 break 하기
			// break;		// 첫 번쨰로 null을 만나면 break 
			continue;		// null을 만나면 다음 명령어를 실행 안 하고 for문 i++ 부터 다시 시작합니다.
		...
		System.out.println();
		System.out.println(sb.toString());		// StringBuilder 타입 객체는 println의 인자로 쓸 때 toString 명시적 싱핼
