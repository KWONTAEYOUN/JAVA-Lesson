...
public class A30HashCodeTest {
    ...
    // hashcod는 참조값(식별값)을 만드는 '해시함수'(수학적)로 생성된 결과입니다.
        System.out.println(iArray);     // 배열의 식별값을 보여줍니다.(16진수 = 8자리)
        System.out.println(iArray.hashCode());      // 해시코드 값을 가져온다.(10진수)
        System.out.println(Integer.toHexString(iArray.hashCode()));     // 10진수를 16진수로 변환

        iArray[3]=99;
        System.out.println("배열 요소값을 변경한 후 해쉬코드 값 : " + iArray.hashCode());
...
// 객체를 메모리에 저장했다. --> 초기에 저장한 데이터를 바꿀 수 있다.(배열) - 가변객체
