...
// socket.png 그림참고 : 클라이언트와 서버 각각 실행하는 메소드가 다르다.
...
public class Server {
    public static void main(String[] args) {
        ...
        try {
            ...
            server.bind(new InetSocketAddress("192.168.30.12", 5050));
            System.out.println(" [서버프로그램 시작] : 연결 요청을 기다리는 중 입니다. ");
            // 클라이언트 연결 요청(connect)의 수락
            socket = server.accept();    // 입출력 스트림을 만들 소켓
            System.out.println(" __ 연결을 수락합니다. ");           // accept 정상 완료되면 실행.

            // 서버가 클라이언트에게 데이터 보내기 : 출력 스트림
            os = socket.getOutputStream();  // 클라이언트와 서버가 연결된 소켓으로 출력 스트림 생성
            // 정수, 실수, 문자열 보내는 스트림
            ...
            // 파일명은 "d:\\pet.jpg" 다운로드 위치는 d드라이브
            // 클라이언트가 보내준 파일명으로 저장하기. (d드라이브 또는 다운도르 폴더)
            ...
            try{server.close(); socket.close(); os.close(); dos.close(); is.close(); dis.close(); bos.close();}catch(IOException e){
