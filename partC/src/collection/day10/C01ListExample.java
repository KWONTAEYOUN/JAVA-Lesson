...
public class C01ListExample {
    public static void main(String[] args) {
        ...
        System.out.println("\n   데이터 삭제");
        strlist.remove("원더걸스");     // renove 메소드 오버로딩
        System.out.println("항목 삭제 후 리스트 : " + strlist);     
        strlist.remove(1);          // 메소드 인자 타입이 int와 String(요소의 타입) 2가지로 동작
        System.out.println("항목 삭제 후 리스트 : " + strlist);     // ArrayList 클래스의 toString 재정의 메소드 실행
        
        // 아래 2개 메소드 비교
        System.out.println("\n   데이터 항목 변경");
        strlist.add(2, "?????");        // 중간 삽입
        System.out.println("항복 변경 후 리스트 : " + strlist);
        strlist.set(2, "!!!!!");        // 수정
        System.out.println("항복 변경 후 리스트 : " + strlist);
        ...
        System.out.println("\n addAII strlist 실행 후 : " + strlist);
        strlist.add("아이브");
        System.out.println("\n 이미 저장되어 있는 값을 추가한 후 : " + strlist);    // 중복값 허용
        // 리스트에서 요소를 찾기(조회)
        System.out.println("\n\n리스트에서 항목 찾기");
        // indexOf
